...
public class UserServiceImpl implements UserService {
    ...
    public User save(User user) {
        ...
        return webClient.post()
                .uri("http://127.0.0.1:8010/api/v1/users")
                .bodyValue(user)
                .retrieve()
    ...
    public User getById(OAuth2AuthorizedClient auth, Long id) {
        return webClient.get()
                .uri("http://127.0.0.1:8010/api/v1/users/" + id)
                .attributes(oauth2AuthorizedClient(auth))
                .retrieve()
                .bodyToMono(User.class)
                .block();
    ...
    @Override
    public User updateObjectById(OAuth2AuthorizedClient auth, Long id, User user) {
        user.setPassword(passwordEncoder.encode(user.getPassword()));
    ...
    @Override
    public User updateParametersById(OAuth2AuthorizedClient auth, Long id, User user) {
        user.setPassword(passwordEncoder.encode(user.getPassword()));
